...
async function checkRealFirebase() {
  try {
    console.log('=== CHECKING REAL FIREBASE DATA ===\n');
    ...
    if (missingCoordinates.length > 0) {
      ...
      // Ask if user wants to add coordinates
      console.log('Would you like to add coordinates to these shops?');
      console.log('Run: node add-coordinates-to-real-firebase.js');
    }
    ...
    console.error('Error checking REAL Firebase:', error);
  }
}

// Run the check
checkRealFirebase().then(() => {
  console.log('\n=== CHECK COMPLETE ===');
  process.exit(0);
}).catch((error) => {
